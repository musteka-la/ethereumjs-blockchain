...
const checkpointData = fromRpc(require('./fixtures/checkpoint-block.json'))
const nextBlockData = fromRpc(require('./fixtures/next-block.json'))
...
test(`should add fork header and reset stale heads`, (t: test.Test) => {
  ...
  blockchain.putHeader(forkHeader, (err?: Error) => {
    ...
    t.test('should update stale head', (tt: test.Test) => {
      t.equals(blockchain._heads['staletest'].toString('hex'), blocks[8].hash().toString('hex'))
      tt.end()
    })
...
test('should add checkpoint block', (t: test.Test) => {
  const checkpointBlock = new Block(checkpointData)
  async.series(
    [
      (cb: Function) => blockchain.putCheckpoint(checkpointBlock, cb),
    ...
    (err, res: any) => {
      ...
      t.equals(checkpointBlock.hash().toString('hex'), res[1].hash().toString('hex'))
      t.end()
    },
  )
})

test('should add blocks after checkpoint', (t: test.Test) => {
  const nextBlock = new Block(nextBlockData)
  async.series(
    [
      (cb: Function) => blockchain.putBlock(nextBlock, cb),
      (cb: Function) => blockchain.getLatestBlock(cb),
    ],
    (err, res: any) => {
      if (err) {
      ...
      t.equals(
        ...
        res[1].hash().toString('hex'),
        'block after checkpoint should be correct',
      )
      t.end()
    },
  )
